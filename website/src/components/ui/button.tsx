...
export function Button({ className, variant = "ghost", render, ...props }: ButtonProps): React.ReactElement {
  return useRender({
    ...
    props: mergeProps<"button">({
      className: cn(
        "relative inline-flex h-7 shrink-0 appearance-none cursor-pointer items-center justify-center gap-1.5 whitespace-nowrap rounded-md border px-2.5 text-xs font-medium outline-none pointer-coarse:h-11 pointer-coarse:min-w-11 focus-visible:ring-2 focus-visible:ring-[var(--accent)] disabled:pointer-events-none disabled:opacity-64 [&_svg]:pointer-events-none [&_svg]:size-3.5 [&_svg]:shrink-0",
        variant === "outline"
