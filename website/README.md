...
The first visit opens the editor with the CLI defaults. Save & Exit shows a local demo composer and the real saved footer; Edit statusline returns to the same saved configuration. Browser storage uses `cxstatusline.playground.settings.v1`; downloads contain the defaults until a saved layout replaces them. The composer is sample-only and sends nothing. Native path import/export remains unavailable in the browser.
...
Use the terminal keyboard controls. Ctrl+S saves while editing; Save & Exit opens the composer preview. On reload, a valid browser-saved configuration opens that preview, while no saved configuration opens the default editor. Path-based import/export and host commands are unavailable; Custom Command only displays its existing placeholder preview. Font detection cannot inspect host fonts.
