...
export class SandboxStatusWidget implements Widget {
    ...
    getDescription(): string {
        return [
            'Shows whether Codex sandbox mode is enabled',
            'Uses Codex session permissions (danger-full-access OFF; read-only and workspace-write ON).'
        ].join('\n');
    }
    ...
    render(item: WidgetItem, context: RenderContext, _settings: Settings): string | null {
        ...
        const permissions = context.data.session?.permissions;
        if (permissions !== 'danger-full-access' && permissions !== 'read-only' && permissions !== 'workspace-write') {
            return null;
        }
        return formatStatus(permissions !== 'danger-full-access', format, nerdFont, item.rawValue ?? false);
    }
