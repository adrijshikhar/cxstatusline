...
import { resolveLiveSpeedMetrics } from '../../utils/speed-tracker';
...
function getSpeedMetricsForWidget(item: WidgetItem, context: RenderContext): SpeedMetrics | null {
    if (!isWidgetSpeedWindowEnabled(item)) {
        if (context.speedMetrics !== undefined) {
            return context.speedMetrics;
        }
        return resolveLiveSpeedMetrics(context);
    }

    const windowSeconds = getWidgetSpeedWindowSeconds(item);
    if (context.windowedSpeedMetrics !== undefined) {
        return context.windowedSpeedMetrics?.[windowSeconds.toString()] ?? null;
    }
    return resolveLiveSpeedMetrics(context, windowSeconds);
}
...
export function renderSpeedWidgetValue(
    ...
    if (speed === null && isHideStateEnabled(item, NO_DATA_HIDEABLE_STATE)) {
        return null;
    }
