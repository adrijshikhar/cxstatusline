...
function chalkLevel(colorLevel: Exclude<ColorLevelString, 'none'>): 1 | 2 | 3 {
    return colorLevel === 'ansi16' ? 1 : colorLevel === 'ansi256' ? 2 : 3;
}

// A renderer owns its color level. Never read or mutate Chalk's process-wide default instance.
function createColorMap(chalk: ChalkInstance): ColorEntry[] {
    return [
...
export const COLOR_MAP: ColorEntry[] = createColorMap(new Chalk({ level: 3 }));
...
export function getChalkColor(colorName: string | undefined, colorLevel: ColorLevelString = 'ansi16', isBackground = false): ChalkInstance | undefined {
    if (!colorName)
        return undefined;
    ...
    const chalk = new Chalk({ level: chalkLevel(colorLevel) });
    ...
    const colorEntry = createColorMap(chalk).find(c => c.name === colorName);
...
export function applyColors(
    ...
    bold?: boolean,
    colorLevel: ColorLevelString = 'ansi16',
    dim?: boolean | 'parens'
): string {
    if (colorLevel === 'none') {
        return text;
...
export function getColorAnsiCode(colorName: string | undefined, colorLevel: ColorLevelString = 'ansi16', isBackground = false): string {
    if (!colorName || colorLevel === 'none')
        return '';
    ...
    const chalkFn = getChalkColor(colorName, colorLevel, isBackground);
    if (!chalkFn)
        return '';
