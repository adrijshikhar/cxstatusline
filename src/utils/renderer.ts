...
import { isHideStateEnabled, MERGE_TARGET_HIDDEN_HIDEABLE_STATE } from '../widgets/shared/hideable';

/** Untrusted strings cross this boundary before renderer-owned ANSI is applied. */
const WIDGET_CONTROL_CHARS = /[\u0000-\u001F\u007F-\u009F]/g;

function sanitizeRawText(text: string): string {
    return text.replace(WIDGET_CONTROL_CHARS, '');
}
...
export function renderStatusLines(settings: Settings, context: RenderContext): string[] {
    ...
    for (let lineIndex = 0; lineIndex < settings.lines.length; lineIndex++) {
        const items = settings.lines[lineIndex];
        if (!items?.length)
            continue;

        const preRendered = preRenderedLines[lineIndex] ?? [];
        const line = renderStatusLine(items, settings, {
            ...context,
            lineIndex,
            globalSeparatorIndex,
            globalPowerlineThemeIndex,
            globalPowerlineStartCapIndex
        }, preRendered, maxWidths);

        if (!getVisibleText(line).trim())
            continue;

        rows.push(colorLevel === 'none' ? line : `\x1b[0m${line}`);
        globalSeparatorIndex = advanceGlobalSeparatorIndex(globalSeparatorIndex, items, preRendered);
        if (settings.powerline.enabled) {
            globalPowerlineStartCapIndex += countPowerlineStartCapSlots(items, preRendered);
            if (settings.powerline.continueThemeAcrossLines)
                globalPowerlineThemeIndex = advanceGlobalPowerlineThemeIndex(globalPowerlineThemeIndex, preRendered);
        }
    }

    return rows.length ? rows.slice(0, 3) : ['codex'];
}
...
function maybeApplyForegroundGradient(
    ...
    return stops && colorLevel !== 'none' ? applyLineGradient(line, stops, colorLevel) : line;
}
...
function resolveEffectiveTerminalWidth(
    ...
    if (flexMode === 'full-until-compact') {
        ...
        const contextPercentage = (context.data.usage?.context_used ?? 0) * 100;
...
function renderPowerlineStatusLine(
    ...
    const separators = sanitizeStringArray(config.separators, ['\uE0B0']);
    ...
    const startCaps = sanitizeStringArray(config.startCaps, []);
    const endCaps = sanitizeStringArray(config.endCaps, []);
    ...
    if (themeName && themeName !== 'custom' && colorLevel !== 'none') {
    ...
    const detectedWidth = context.terminalWidth;
    ...
    for (let i = 0; i < filteredWidgets.length; i++) {
        ...
        if (widgetText) {
            ...
            const padding = sanitizeRawText(settings.defaultPadding ?? '');
    ...
    for (let i = 0; i < widgetElements.length; i++) {
        ...
        if (textGradientStops && gradientColorLevel) {
...
function isSpacingSeparator(widget: WidgetItem | undefined, defaultSeparator: string | undefined): boolean {
    return widget?.type === 'separator'
        && sanitizeRawText(widget.character ?? defaultSeparator ?? '|').trim().length === 0;
}
    ...
    for (let i = 0; i < segment.length; i++) {
        ...
        for (let position = 0; position < chain.length; position++) {
            ...
            if (!element
                ...
                || !isHideStateEnabled(element.widget, MERGE_TARGET_HIDDEN_HIDEABLE_STATE)) {
                continue;
            }
...
export function calculateMaxWidthsFromPreRendered(
    ...
    const defaultPadding = sanitizeRawText(settings.defaultPadding ?? '');
...
export function renderStatusLine(
    ...
    elements.forEach((elem, index) => {
        ...
        if (elem.type === 'separator' || elem.type === 'flex-separator') {
            ...
            if (padding && (elem.widget?.backgroundColor || hasColorOverride)) {
                ...
                const leadingPadding = omitLeadingPadding || !sideLeadingPadding
                    ? ''
                    : colorLevel === 'none' ? sideLeadingPadding : `\x1b[0m${sideLeadingPadding}\x1b[0m`;
                const trailingPadding = omitTrailingPadding || !sideTrailingPadding
                    ? ''
                    : colorLevel === 'none' ? sideTrailingPadding : `\x1b[0m${sideTrailingPadding}\x1b[0m`;
    ...
    if (hasFlexSeparator && terminalWidth) {
        ...
        if (hasFlexSeparator && !terminalWidth) {
            ...
            statusLine = finalElements.map(e => e === 'FLEX' ? applyColorsWithOverride(' | ', 'brightBlack') : e).join('');
        } else {
    ...
    return colorLevel === 'none' ? getVisibleText(statusLine) : statusLine;
}
