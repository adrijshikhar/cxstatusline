...
export function createInstallProgressTracker(
  ...
  const onProgress = (loaded: number, total: number | null) => {
    const now = Date.now();
    if (!downloadStart) downloadStart = now;
    ...
    if (total !== null && total > 0) {
      ...
      if (isTTY) {
        ...
        const text = `\r    ${bar} ${chalk.bold(String(pct).padStart(3))}%  ${loadedStr} / ${totalStr} (${chalk.dim(speedStr)})`;
        io.write(text);
        hasProgressBar = true;
      } else {
      ...
      if (isTTY) {
        io.write(`\r    ${symbols.activeBullet} ${formatBytes(loaded)} downloaded (${chalk.dim(speedStr)})`);
        hasProgressBar = true;
      }
    }
  };
