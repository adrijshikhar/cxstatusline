...
describe("widget editor dispatch", () => {
  ...
  test("custom-command: p toggles preserve, t opens the timeout editor, e opens the command editor, preview never runs it", async () => {
    ...
    try {
      ...
      await app.write("f");
      expect(app.lastFrame()).toContain("Enter refresh interval in ms");
      expect(app.lastFrame()).toContain("Output is shared by every session in the same directory");
      await app.write("5000");
      await app.write("\r");
      expect(app.lastFrame()).toContain("refresh: 5000ms");
      await app.write("f");
      await app.write("\x7f");
      await app.write("\x7f");
      await app.write("\x7f");
      await app.write("\x7f");
      await app.write("\r");
      expect(app.lastFrame()).not.toContain("refresh:");
      await app.write("e");
